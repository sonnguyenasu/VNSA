...
class NormalizeText():
    def __init__(self):
        self.not_list=['không','vô','chẳng','đếch','chưa','đéo','kém','not']
        with codecs.open('/content/drive/MyDrive/VBDI/ML/Final Project/pos.txt', 'r', encoding='UTF-8') as f:
            pos = f.readlines()
        self.pos_list = [n.replace('\n', '') for n in pos]
        with codecs.open('/content/drive/MyDrive/VBDI/ML/Final Project/neg.txt', 'r', encoding='UTF-8') as f:
            nag = f.readlines()
        self.nag_list = [n.replace('\n', '') for n in nag]
    ...
    def normalize_text(self,text):
        ...
        for i in range(len_text):
            cp_text = texts[i]
            if cp_text in self.not_list: # Xử lý vấn đề phủ định (VD: áo này chẳng đẹp--> áo này notpos)
                numb_word = 2 if len_text - i - 1 >= 4 else len_text - i - 1

                for j in range(numb_word):
                    if texts[i + j + 1] in self.pos_list:
                        texts[i] = 'notpos'
                        texts[i + j + 1] = ''

                    if texts[i + j + 1] in self.nag_list:
                        texts[i] = 'notnag'
                        texts[i + j + 1] = ''
            else: #Thêm feature cho những sentiment words (áo này đẹp--> áo này đẹp positive)
                if cp_text in self.pos_list:
                    texts.append('positive')
                elif cp_text in self.nag_list:
    ...
    def transform(self, x):
        list_of_text = []
        for text in x:
            list_of_text.append(self.normalize_text(text))
        list_of_test = np.array(list_of_text)
        return list_of_text